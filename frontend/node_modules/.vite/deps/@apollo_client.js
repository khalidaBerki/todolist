...
function shouldInclude(_a2, variables) {
  ...
  return getInclusionDirectives(directives).every(function(_a3) {
    ...
    if (ifArgument.value.kind === "Variable") {
      ...
      invariant2(evaledValue !== void 0, 70, directive.name.value);
    } else {
...
function getInclusionDirectives(directives) {
  ...
  if (directives && directives.length) {
    directives.forEach(function(directive) {
      ...
      invariant2(directiveArguments && directiveArguments.length === 1, 71, directiveName);
      var ifArgument = directiveArguments[0];
      invariant2(ifArgument.name && ifArgument.name.value === "if", 72, directiveName);
      var ifValue = ifArgument.value;
      invariant2(ifValue && (ifValue.kind === "Variable" || ifValue.kind === "BooleanValue"), 73, directiveName);
...
function getFragmentQueryDocument(document, fragmentName) {
  ...
  if (typeof actualFragmentName === "undefined") {
    invariant2(fragments.length === 1, 75, fragments.length);
...
function getFragmentFromSelection(selection, fragmentMap) {
  switch (selection.kind) {
    ...
    case "FragmentSpread": {
      ...
      invariant2(fragment, 76, fragmentName);
...
function checkDocument(doc) {
  ...
  invariant2(operations.length <= 1, 79, operations.length);
  return doc;
}
...
function getQueryDefinition(doc) {
  ...
  invariant2(queryDef && queryDef.operation === "query", 80);
  return queryDef;
}
function getFragmentDefinition(doc) {
  invariant2(doc.kind === "Document", 81);
  invariant2(doc.definitions.length <= 1, 82);
  var fragmentDef = doc.definitions[0];
  invariant2(fragmentDef.kind === "FragmentDefinition", 83);
  return fragmentDef;
}
      ...
      if (this.cached) {
        ...
        this.performWork = wrap3(DocumentTransform2.prototype.performWork.bind(this), {
          makeCacheKey: function(document) {
            ...
            if (cacheKeys) {
              invariant2(Array.isArray(cacheKeys), 69);
...
function removeDirectivesFromDocument(directives, doc) {
  ...
  var getInUse = function(ancestors) {
    ...
    globalThis.__DEV__ !== false && invariant2.error(86);
    return null;
  };
...
function toPromise(observable) {
  ...
  return new Promise(function(resolve, reject) {
    observable.subscribe({
      next: function(data) {
        if (completed) {
          globalThis.__DEV__ !== false && invariant2.warn(43);
        } else {
          completed = true;
          resolve(data);
        }
  ...
  function() {
    ...
    ApolloLink2.concat = function(first, second) {
      ...
      if (isTerminating(firstLink)) {
        globalThis.__DEV__ !== false && invariant2.warn(36, firstLink);
        return firstLink;
      }
  ...
  return new ApolloLink(function(operation) {
    ...
    if (hasDefer || isSubscription) {
      ...
      if (isSubscription && hasDefer) {
        globalThis.__DEV__ !== false && invariant2.warn(39);
      }
      if (isSubscription) {
    ...
    ObservableQuery2.prototype.subscribeToMore = function(options) {
      ...
      var subscription = this.queryManager.startGraphQLSubscription({
        ...
        error: function(err) {
          ...
          globalThis.__DEV__ !== false && invariant2.error(22, err);
        }
      });
...
function defaultSubscriptionObserverErrorCallback(error) {
  globalThis.__DEV__ !== false && invariant2.error(24, error.message, error.stack);
}
function logMissingFieldErrors(missing) {
  if (globalThis.__DEV__ !== false && missing) {
    globalThis.__DEV__ !== false && invariant2.debug(25, missing);
  }
}
...
var QueryManager = (
  ...
  function() {
    ...
    QueryManager2.prototype.mutate = function(_a2) {
      return __awaiter(this, arguments, void 0, function(_b) {
        ...
        return __generator(this, function(_j) {
          switch (_j.label) {
            case 0:
              invariant2(mutation, 27);
              invariant2(fetchPolicy === "network-only" || fetchPolicy === "no-cache", 28);
    ...
    QueryManager2.prototype.query = function(options, queryId) {
      ...
      invariant2(options.query, 29);
      invariant2(options.query.kind === "Document", 30);
      invariant2(!options.returnPartialData, 31);
      invariant2(!options.pollInterval, 32);
    ...
    QueryManager2.prototype.getObservableQueries = function(include) {
      ...
      if (globalThis.__DEV__ !== false && queryNamesAndDocs.size) {
        queryNamesAndDocs.forEach(function(included, nameOrDoc) {
          if (!included) {
            globalThis.__DEV__ !== false && invariant2.warn(typeof nameOrDoc === "string" ? 34 : 35, nameOrDoc);
          }
        });
      }
      return queries;
...
var ApolloClient = (
  ...
  function() {
    function ApolloClient2(options) {
      ...
      var uri = options.uri, credentials = options.credentials, headers = options.headers, cache2 = options.cache, documentTransform = options.documentTransform, _b = options.ssrMode, ssrMode = _b === void 0 ? false : _b, _c = options.ssrForceFetchDelay, ssrForceFetchDelay = _c === void 0 ? 0 : _c, connectToDevTools = options.connectToDevTools, _d = options.queryDeduplication, queryDeduplication = _d === void 0 ? true : _d, defaultOptions2 = options.defaultOptions, defaultContext = options.defaultContext, _e = options.assumeImmutableResults, assumeImmutableResults = _e === void 0 ? cache2.assumeImmutableResults : _e, resolvers = options.resolvers, typeDefs = options.typeDefs, fragmentMatcher = options.fragmentMatcher, clientAwarenessName = options.name, clientAwarenessVersion = options.version, devtools = options.devtools;
...
function getApolloContext() {
  invariant2("createContext" in React, 46);
...
var ApolloConsumer = function(props) {
  ...
  return React2.createElement(ApolloContext.Consumer, null, function(context) {
    invariant2(context && context.client, 45);
...
var useSyncExternalStore = realHook || function(subscribe2, getSnapshot, getServerSnapshot) {
  ...
  if (
    ...
    globalThis.__DEV__ !== false && invariant2.error(60);
  }
...
function parser(document) {
  ...
  invariant2(!!document && !!document.kind, 62, document);
  var fragments = [];
  var queries = [];
  var mutations = [];
  var subscriptions = [];
  ...
  invariant2(!fragments.length || queries.length || mutations.length || subscriptions.length, 63);
  invariant2(
  ...
  invariant2(definitions.length === 1, 65, document, definitions.length);
...
function useLazyQuery(query, options) {
  ...
  var execute3 = React11.useCallback(function(executeOptions) {
  ...
  var executeRef = React11.useRef(execute3);
...
function useSubscription(subscription, options) {
  ...
  if (!hasIssuedDeprecationWarningRef.current) {
    ...
    if (options.onSubscriptionData) {
      globalThis.__DEV__ !== false && invariant2.warn(options.onData ? 53 : 54);
    }
    if (options.onSubscriptionComplete) {
      globalThis.__DEV__ !== false && invariant2.warn(options.onComplete ? 55 : 56);
    }
  }
  ...
  var restart = React13.useCallback(function() {
    invariant2(!optionsRef.current.skip, 57);
...
function assertWrappedQueryRef(queryRef) {
  invariant2(!queryRef || QUERY_REFERENCE_SYMBOL in queryRef, 61);
}
...
function validateFetchPolicy(fetchPolicy) {
  ...
  invariant2(supportedFetchPolicies.includes(fetchPolicy), 58, fetchPolicy);
}
function validatePartialDataReturn(fetchPolicy, returnPartialData) {
  if (fetchPolicy === "no-cache" && returnPartialData) {
    globalThis.__DEV__ !== false && invariant2.warn(59);
  }
}
...
function useLoadableQuery(query, options) {
  ...
  var loadQuery = React18.useCallback(function() {
    ...
    invariant2(!calledDuringRender(), 51);
  ...
  var subscribeToMore = React18.useCallback(function(options2) {
    invariant2(internalQueryRef, 52);
